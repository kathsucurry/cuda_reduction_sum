...
__global__ void batched_interleaved_address_2(
...
template <size_t NUM_THREADS>
void launch_batched_interleaved_address_2(
    float* Y,
    float const* X,
    size_t batch_size,
    size_t num_elements_per_batch,
    ...
    batched_interleaved_address_2<NUM_THREADS>
...
template <size_t NUM_THREADS>
void profile_interleaved_address_2(
    size_t string_width,
    std::vector<float> Y,
    float* Y_d,
    float *X_d,
    ...
    std::cout << "Batched reduce sum - INTERLEAVED ADDRESSING #2" << std::endl;
    profile_batched_kernel(
        launch_batched_interleaved_address_2<NUM_THREADS>,
